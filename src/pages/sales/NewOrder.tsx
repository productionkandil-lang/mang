...
export default function NewOrder() {
  ...
  const [formData, setFormData] = useState({
    ...
    paintColors: '',
    paintType: '',
    paintNewColorDetails: '',
    paintImages: [] as string[],
    paintPrice: '',

    // Upholstery
    hasUpholsteryRenovation: false,
    uphWorkType: '',
    uphSpecs: '',
    uphDiamondType: '',
    uphThreadColor: '',
    ...
    uphPipingColor: '',
    uphNotes: '',
    uphLogoEmbroideryColor: '',
    uphLogoPlaces: '',
    uphLogoType: '',
    uphLogoImage: [] as string[],
    uphPrice: '',
    hasUpholsteryManufacturing: false,
    uphManuWorkType: '',
    uphManuSpecs: '',
    uphManuDiamondType: '',
    uphManuThreadColor: '',
    ...
    uphManuPiecesCount: '',
    uphManuColorCode: '',
    uphManuImages: [] as string[],
    uphManuPrimaryColor: '',
    uphManuSecondaryColor: '',
    uphManuSideColor: '',
    uphManuPipingColor: '',
    uphManuNotes: '',
    uphManuLogoEmbroideryColor: '',
    uphManuLogoPlaces: '',
    uphManuLogoType: '',
    uphManuLogoImage: [] as string[],
    uphManuPrice: '',
    ...
    floorMiddleColor: '',
    floorBottomColor: '',
    floorBoatImages: [] as string[],
    floorDesignImages: [] as string[],
    floorHasLogo: false,
    floorLogoDetails: '',
    floorLogoType: '',
    floorLogoImage: [] as string[],
    floorPrice: '',
  ...
  const renderWorkTypeSelect = (deptKey: string, fieldName: string, label: string) => {
    ...
    return (
      <div className="space-y-2">
        <Label>{label}</Label>
        <Select value={(formData as any)[fieldName]} onValueChange={(val) => setFormData(prev => ({ ...prev, [fieldName]: val }))}>
      ...
      return (
        <div key={f.id} className="space-y-2 col-span-full md:col-span-1">
          ...
          {f.field_type === 'text' && (
            <Input 
              value={value} 
              onChange={e => setFormData(prev => ({ ...prev, [f.field_key]: e.target.value }))} 
          ...
          {f.field_type === 'select' && f.options && (
            <Select value={value} onValueChange={val => setFormData(prev => ({ ...prev, [f.field_key]: val }))} required={f.is_required}>
          ...
          {f.field_type === 'image' && (
            <ImageUpload 
              ...
              onChange={urls => setFormData(prev => ({ ...prev, [f.field_key]: urls }))} 
            />
          )}
        </div>
      );
    ...
    try {
      ...
      const { data: project, error: projectError } = await supabase
        ...
        .insert({
          ...
          product_details,
          status: 'في انتظار اعتماد التصميم',
      ...
      // Fetch base tasks
      const { data: refTasks, error: refError } = await supabase
      ...
      if (refTasks && refTasks.length > 0) {
        // Map UI selections to option names
        ...
        // Filter tasks that match active departments and selected options
        const filteredTasks = refTasks.filter(rt => {
          if (!activeDepartments.includes(rt.department)) return false;
          // General tasks without specific option requirement are always added
          if (!rt.option_name || rt.option_name.trim() === '') return true;
          // Check if the selected options for this department include this task's option
          const selectedForDept = deptOptions[rt.department] || [];
          return selectedForDept.includes(rt.option_name);
        });
        
        // If we found any relevant tasks, insert them
        if (filteredTasks.length > 0) {
          ...
          // If no specific tasks matched, insert at least one dummy task per active department
          const fallbackTasks = Array.from(new Set(activeDepartments)).map((dep, idx) => ({
            project_id: project.id,
            department: dep,
            name: `مهام ${dep} الأساسية`,
          ...
          if (fallbackTasks.length > 0) {
            await supabase.from('project_tasks').insert(fallbackTasks);
          }
        }
      }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">إدخال أوردر جديد</h2>
        <p className="text-muted-foreground">نموذج تفصيلي لطلبات الإنتاج والصيانة.</p>
      </div>
...
<AccordionItem value="upholstery_manufacturing" className="border rounded-lg bg-card px-4">
        ...
        <div className="flex justify-end gap-4 mt-8 pt-6 border-t">
          ...
          <Button type="submit" disabled={loading} className="w-48 text-lg font-bold">
            {loading ? 'جاري الحفظ...' : 'إنشاء وتأكيد الأوردر'}
          </Button>
        </div>
      </form>
    </div>
